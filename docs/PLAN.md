...
- **2026-08-28 — Password reset (`password-reset`) ships without a chosen email provider, but production requires one to boot.** `POST /auth/forgot-password` sends its link through a `MailSender` abstraction; the default `LoggingMailSender` writes the link to the application log instead of transmitting it, and is the only implementation active in `dev` and in tests, so the whole flow works and is fully tested before any SMTP account exists. Production is different: `application-prod.yml` gives `app.mail.provider` no default (unlike `dev`/tests, where `logging` is the default), so the backend refuses to start in the `prod` profile until `APP_MAIL_PROVIDER=smtp` and all four SMTP-specific `APP_MAIL_*` Render variables (see `docs/deployment.md`) are set — a production deployment cannot silently fall back to logging live, unexpired password-reset tokens instead of emailing them. See `openspec/changes/archive/2026-08-28-password-reset/design.md` "The mail abstraction and the development default".
- **2026-08-28 — Self-registration (`user-signup`) is open by default.** `POST /auth/register` lets anyone create a `USER` account with no invite or approval step. Because `certificates` gives every authenticated user the same shared visibility over the whole table (not just their own records), this is a real widening of exposure, not just an auth convenience — see `openspec/changes/archive/2026-08-28-user-signup/design.md` "Open registration is a decision, not a default". `app.auth.registration-enabled` (default `true`) is the escape hatch if that turns out to be the wrong model once strangers can sign up; the fix at that point is a scoping change to `certificates`, not a patch to this flag.
