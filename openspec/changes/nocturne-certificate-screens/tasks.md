...
- [ ] 7.2 Against the running backend: create, edit and delete a certificate; preview and download its PDF; import a CSV with deliberate bad rows and open the error report. **Not done.** The backend was up, but signing in needs a password and there is no seeded account, so every screen was driven against a temporary mock interceptor instead (since removed). The import path was exercised end to end that way — a 40-row response with 6 failures rendered the counters and the error grid, and the error report produced the correct RFC 4180 CSV, verified byte for byte. What remains unverified is that the real backend answers these screens the way the mock did.
