...
The `batch_imports` audit requirement's own scenario is "a CSV batch import **completes**, regardless of how many rows failed" — i.e. it made it through the row-count gate and was actually processed, not that literally every HTTP request (including a flatly oversized upload rejected before parsing) gets an audit row. An audit row is persisted once, after the full batch is processed. The write happens through a single `BatchImportRepository.save()` call, which Spring Data already wraps in its own transaction — a separate `@Transactional` on the calling method would be a no-op anyway, since it's invoked by plain self-invocation and never passes through the proxy. Persisting the audit record can never retroactively undo certificates already committed (those are independent, already-committed transactions by this point); conversely, a failure persisting the audit row is logged and swallowed rather than surfaced as an error response, since the import itself already succeeded and a 500 here would misleadingly suggest otherwise.

## CSV header validation

The uploaded file's actual header row is read and compared against the documented column order (`docs/api-reference.md`) before any row is processed. A mismatched or reordered header is rejected with 400 rather than silently mapping the wrong columns into the wrong fields — a reordered header that still parsed successfully by position would create certificates with swapped data and no error reported, which is worse than an outright rejection.
