...
- [x] 1.2 `.html`: back-to-list link, spinner while loading, error message on failure, `<iframe>` (no `sandbox` attribute — see design.md) bound to a sanitized blob URL plus a Download button once loaded.
...
- [x] 3.1 `certificate-preview-page.component.spec.ts`: shows the spinner while the fetch is pending; shows the iframe with a blob URL once loaded; shows an error state on a failed certificate lookup and on a failed PDF fetch; the Download button triggers a save; the back link points to the list; the blob URL is revoked on destroy.
