...
public class RefreshTokenService {
     ...
     *
     * <p>Looks the kept token up first and requires it to be a live, unrevoked token belonging to
     * {@code user}: without this, a stale or mismatched token would silently match nothing in the
     * bulk update below, and "except" would revoke every session, including the caller's own.
     */
    @Transactional
    public void revokeAllExcept(User user, String rawTokenToKeep) {
        RefreshToken kept =
                refreshTokenRepository
                        ...
                        .filter(token -> token.getUser().getId().equals(user.getId()) && !token.isRevoked())
