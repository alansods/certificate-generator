...
- **2026-08-28 — Password reset (`password-reset`) ships without a chosen email provider.** `POST /auth/forgot-password` sends its link through a `MailSender` abstraction; the default `LoggingMailSender` writes the link to the application log instead of transmitting it, so the whole flow works and is fully tested before any SMTP account exists. `app.mail.provider=smtp` plus `docs/deployment.md`'s five `APP_MAIL_*` Render variables switch to a real provider later — startup fails fast if that flag is set without all four SMTP properties, so a half-configured deployment can't silently keep logging reset links in production. See `openspec/changes/archive/2026-08-28-password-reset/design.md` "The mail abstraction and the development default".
