...
The system SHALL let any authenticated user download a certificate's PDF from its row, and SHALL let only an ADMIN delete a certificate from its row.

#### Scenario: Download PDF
- **WHEN** a user selects the download action on a row
- **THEN** `GET /api/v1/certificates/{id}/pdf` is requested and the returned file is offered to the user
